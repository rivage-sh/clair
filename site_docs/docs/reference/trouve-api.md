...
| `resolved_sql` | `str` | SQL with all placeholder tokens replaced by real full_names. Empty for a `PandasTrouve`. |
| `resolved_transform` | `str` | The source text of the transform function. Empty for a SQL `Trouve`. |
...
| Aspect | Detail |
|--------|--------|
| Dependencies | Each Trouve in `inputs` becomes an upstream dependency. Clair binds them to the transform parameters by position. |
