...
| `physical_name` | `str` | Fully-qualified Snowflake name (`database.schema.table`). Raises `RuntimeError` if not compiled. |
...
|-----------|------|-------------|
| `physical_name` | `str` | The routed Snowflake name. clair uses it in the SQL and the DDL. |
| `logical_name` | `str` | The name from the file system. clair uses it for the DAG edges and the selectors. |
| `resolved_sql` | `str` | The SQL. clair replaced each placeholder token with a real physical_name. It is empty for a `PandasTrouve`. |
...
At discovery, clair replaces every placeholder with the real `physical_name` of the Trouve. Thus clair builds the dependency graph and resolves the SQL names.
