...
Clair ships no concrete ``RoutingEntry``. A user writes one. These two entries
give the tests the same two shapes that a user writes most often.
"""
...
class DatabaseOverrideRouting(RoutingEntry):
    """Send every non-SOURCE Trouve to one database."""
