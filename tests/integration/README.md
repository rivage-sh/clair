...
```bash
UV_PROJECT_ENVIRONMENT=venv uv run python -m tests.integration.setup
```
...
```bash
UV_PROJECT_ENVIRONMENT=venv uv run python -m tests.integration.clean_up --schema-name pr_42
```
...
UV_PROJECT_ENVIRONMENT=venv uv run python -m tests.integration.setup
UV_PROJECT_ENVIRONMENT=venv uv run pytest tests/integration -m integration -v -n 6
```
