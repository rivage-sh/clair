...
def _single_table_dag(
    ...
    if run_config is None:
        trouve = Trouve(sql="SELECT 1 AS id", type=trouve_type)
    else:
        trouve = Trouve(sql="SELECT 1 AS id", type=trouve_type, run_config=run_config)
    _compile(trouve, "db.s.orders")
    dag = build_dag([trouve])
    return dag, get_executable_nodes(dag)


class TestStagingRunner:
    def test_build_targets_staging_and_promotes_after_passing_tests(self):
        ...
        results = list(run_project(
            ...
            run_id=RUN_ID, after_node_success=on_success, use_staging=True,
        ))
    ...
    def test_failing_tests_retain_the_candidate_and_leave_target_untouched(self):
        ...
        results = list(run_project(
            dag, selected, adapter,
            run_id=RUN_ID, after_node_success=lambda _n, _p: False, use_staging=True,
        ))
    ...
    def test_promotion_is_identical_when_the_target_does_not_exist(self):
        ...
        list(run_project(
            dag, selected, adapter,
            run_id=RUN_ID, after_node_success=lambda _n, _p: True, use_staging=True,
        ))
    ...
    def test_failed_promotion_retains_staging_and_reports_failure(self):
        dag, selected = _single_table_dag()
        adapter, _executed = _make_adapter(fail_on="CLONE")
        ...
        assert "stays at" in results[0].error
    ...
    def test_incremental_clones_target_into_staging_first(self):
        ...
        list(run_project(
            ...
            after_node_success=lambda _n, _p: True, use_staging=True,
        ))
    ...
    def test_downstream_reads_the_promoted_name_of_its_upstream(self):
        ...
        downstream_staging = _staging_name("db.s.downstream", RUN_ID)
        ...
        assert STAGING_SUFFIX not in build_sql.split("FROM")[1]

    def test_a_staged_run_without_tests_is_rejected(self):
        dag, selected = _single_table_dag()
        adapter, _ = _make_adapter()

        with pytest.raises(RunError, match="needs the tests"):
            list(run_project(dag, selected, adapter, run_id=RUN_ID, use_staging=True))
...
class TestNonStrictUnchanged:
    def test_target_is_written_directly_and_tested_afterwards(self):
        ...
        assert not any(STAGING_SUFFIX in sql for sql in executed)

    def test_failing_tests_do_not_fail_the_node_itself(self):
        """Without staging the write already happened, so only the dependents stop."""
        dag, selected = _single_table_dag()
        adapter, _ = _make_adapter()
...
class TestPhysicalNameOverride:
    ...
    def test_sql_queries_the_override(self):
        ...
        run_tests(dag, ["db.s.orders"], adapter, query_addresses={"db.s.orders": "db.s.staging"})
    ...
    def test_sampling_applies_to_the_override(self):
        ...
        run_tests(
            ...
            use_sample=True,
            query_addresses={"db.s.orders": "db.s.staging"},
        )
